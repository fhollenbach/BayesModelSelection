...
data_sub <- data %>% select(c(hmccode, year, D_polity_s_interp, L_Polity_s_interp, L_Fiscal_Rel_interp, D_Fiscal_Rel_Interp, L_D_Fiscal_Rel_Interp, L_logGDPPERCAP, L_CivilWar, L_REGION_DEM_DIFFUSE, L_WORLD_DEM_DIFFUSE,  D_GDPPERCAP, D_RegionalDiffusion, D_WORLD_DEM_DIFFUSE, very_unequal_utip, total_fuel_income_pc,total_resources_income_pc, gdp_verylow_firstoil_year, reserves_interp_area))

data_sub <- data_sub %>% mutate(L_very_unequal_utip = lag(very_unequal_utip, order_by = year),
                                L_total_fuel_income_pc = lag(total_fuel_income_pc, order_by = year),
                                L_total_resources_income_pc = lag(total_resources_income_pc, order_by = year),
                                L_reserves_interp_area = lag(reserves_interp_area, order_by = year)) %>% select(-c(very_unequal_utip, total_fuel_income_pc,total_resources_income_pc, reserves_interp_area))


data_imp <- data_sub[complete.cases(select(data_sub, -c(gdp_verylow_firstoil_year, L_very_unequal_utip, L_total_fuel_income_pc, L_total_resources_income_pc, L_reserves_interp_area))), ]
...
model.data.ineq <- model.data %>% select(-c(total_fuel_income_pc,total_resources_income_pc, gdp_verylow_firstoil_year, reserves_interp_area)) %>% mutate(
                                                                                                                                                      ...
                                                                                                                                                      unequal_D_Fiscal_Rel_Interp = L_very_unequal_utip * D_Fiscal_Rel_Interp)
...
model.data.full <- model.data.full %>% select(-c(D_Fiscal_Rel_Interp, L_D_Fiscal_Rel_Interp, D_GDPPERCAP, D_RegionalDiffusion, D_WORLD_DEM_DIFFUSE,total_resources_income_pc, total_fuel_income_pc, reserves_interp_area))                                                                 
